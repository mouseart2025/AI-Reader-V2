...
**Post-analysis background tasks (non-fatal)**: on completion, `_schedule_post_analysis()` fires two background tasks. The geo chain (`_run_geo_pipeline`) is serialized — `_auto_rebuild_hierarchy` (GeoOrchestrator v2, no LLM, <1s) must finish before `_auto_spatial_completion` (LLM, 60–300s) starts, because both do load-modify-save whole-document writes to `world_structures`; concurrent execution let spatial completion overwrite the rebuilt hierarchy with its stale pre-rebuild baseline (last-writer-wins race, Epic 6). Entity resolution (Epic 2) stays concurrent — it never writes `world_structures`. Both rebuild chains (auto + `POST /rebuild-hierarchy-v2`) share `build_default_orchestrator()`.
...
**LLM cluster canonical anchoring + re-election** (`entity_resolver.py`, issue #70): canonical selection for `llm_merge` clusters uses tiered semantic anchoring — a name must have appeared *as a person mention* in chapter facts (`mention` tier) or as a person in the entity dictionary; raw corpus substring match is only a last-resort fallback and is audit-tagged. `reelect_llm_merge_canonicals()` runs before LLM clustering on every `resolve_novel` pass and re-elects a stronger canonical when one exists among the override's recorded members (strictly greater mention-chapter count, or current canonical no longer in the eligible pool); ties never flip (idempotent), manually locked names are skipped, and every re-election lands in `entity_resolution_log` as `canonical_reelection`.
...
- **Testing**: Backend pytest (498 tests: chapter splitter, fact validator, alias resolver, name authority, naming pipeline integration, golden standard regression, hierarchy validator, orphan-close, export, settings, cost tracking). Frontend vitest (novelPaths, spatialLabels). CI via `.github/workflows/test.yml`. Run: `cd backend && uv run pytest tests/` / `cd frontend && npx vitest run`
