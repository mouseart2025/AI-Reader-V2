...
支持本地 Ollama 和 10 大云端 API 供应商（含 Anthropic/Claude 原生协议）两种 LLM 推理模式，数据全部存储在本机。

## 功能概览

- **书架管理** — 上传 .txt/.md 小说（拖拽上传 + 上传进度条），自动章节切分，重复检测，搜索过滤（按书名/作者），四种排序（最近/书名/章节/字数），卡片显示分析完成状态 + 阅读位置 + 上次阅读时间，删除确认显示关联数据量，缓存过期自动提示，键盘快捷键（N/Esc//），数据导入/导出/全量备份恢复，novelStore 集中管理，数据库事务保护
- **实体预扫描** — 分析前自动统计扫描 + LLM 分类，生成高频实体词典，注入分析流水线提升提取质量
- **智能阅读** — 实体高亮（人物/地点/物品/组织/概念，可开关 + 按类型过滤），别名自动高亮并解析到同一人物卡片，阅读进度记忆，书签系统（添加/删除/跳转），滚动进度条，章节字数显示，快捷键导航（←/→/H/S/Esc），场景面板搜索过滤（角色 + 情绪基调），章节预加载，实体卡片缓存。人物卡片关系按分类分组显示（血亲/亲密/师承/社交/敌对），多条证据可折叠查看
...
- **知识图谱** — 力导向人物关系图，按章节范围探索关系演变，节点合并别名显示，关系类型自动规范化（70+ 同义映射），边颜色按六大分类着色（血亲/亲密/师承/社交/敌对/其他），分类过滤切换芯片（带数量统计），组织归属智能推断（org_events + 地点关联回退），关系统计图例（可点击过滤），暗色模式完整适配，弱边虚线，密集网络自动稀疏
- **多层级世界地图** — 宏观区域划分（如四大部洲）、多空间层（天界/冥界/洞府副本）、传送门连接、区域内约束求解布局、程序化地形生成（Whittaker 二维生物群落矩阵 + Lloyd 松弛 + OpenSimplex 噪声 + 河流水系网络）、人物轨迹动画（渐进式路径绘制 + 脉冲标记 + 三档变速 + 自动跟随 + 停留时长圆点 + 章节标注）、标签智能防碰撞（8 候选锚点尝试 + 模拟退火全局优化导出）、三态战争迷雾、用户可编辑世界结构、16:9 宽画布、智能副本合并、地理上下文面板（Tab 切换）、场景转换分析增强地点层级、两步层级重建（预览变更→确认应用）、rough.js 手绘风格地图渲染（领地 hachure 填充 + 河流手绘笔触 + 海岸线有机轮廓 + 晕影边缘效果）、地形语义纹理层（山脉三角/河流波浪/森林树丛/沙漠点阵/洞穴钟乳石，按地点 icon 自动散布）、暗色层专属大气纹理（天界星空/冥界岩雾/海底波浪/副本漩涡/灵界紫雾）、区域名称弯曲文字（SVG textPath 沿弧线排列）、地点点击直接打开实体卡片、高清 PNG 地图导出
- **时间线** — 多泳道事件时间线，按重要度/类型筛选，智能降噪（过滤"出现"类物品事件 + ≥3章角色登场门槛 + 噪声类型默认隐藏），关系变化事件自动生成（新建/转变关系），场景情绪基调链接（紧张/悲伤/欢乐等 9 种彩色标签），泳道最少事件过滤，低重要度章节自动折叠
- **势力图** — 组织架构与势力关系网络
- **百科全书** — 分类浏览所有概念/功法/物品，全文搜索，地点层级树形视图
- **智能问答** — 流式对话，RAG 检索增强，答案来源溯源，对话历史管理
- **多 LLM 后端** — 支持本地 Ollama 和云端 10 大供应商（DeepSeek、MiniMax、阿里云百炼、Moonshot/Kimi、智谱 GLM、SiliconFlow、零一万物、OpenAI、Google Gemini、Anthropic/Claude），Anthropic 原生协议（x-api-key + /v1/messages）独立客户端，供应商下拉含预设模型列表，Token 预算根据模型上下文窗口自动缩放（8K~200K 线性插值）
- **分析质量监控** — 实时计时统计与 ETA 预估、章节截断/分段质量追踪、失败章节自动重试、完成后持久化计时摘要
- **性能测试** — 一键测试当前模型性能，基于黄金标准的输出质量评估（实体识别率 + 关系识别率），测试历史记录保存，方便不同模型/时间对比
- **设定集导出** — 四种格式（Markdown/Word/Excel/PDF），两种模板（通用/网文作者套件，全格式可选），章节范围选择，关系链震荡去重，物品噪声过滤，Timeline 500 条完整事件（角色登场/物品交接自动过滤），导出进度反馈，文件名含模板名+章节范围
- **数据管理** — 全量导出/导入（v3 格式含书签/地图调整/世界观修正），file_hash 重复检测，批量对话导出，全量备份（动态版本号），分析数据清除，环境健康检查
...
5. **程序化地形** — Voronoi 区域划分 + Lloyd 松弛优化 + OpenSimplex 噪声生成地形底图，Whittaker 二维生物群落矩阵（海拔×湿度→颜色双线性插值），河流水系网络（梯度下降 + 横向扰动）
...
| Epic 26 | 分析流水线增强 — 实时计时/ETA + 质量追踪 + 失败自动重试 + 性能测试质量评估与历史记录 | 2 | done |
| Epic 27 | 云端 API 扩展 — 多供应商支持（+7 供应商）+ Anthropic/Claude 原生客户端 + 前端模型预设选择器 | 3 | done |
| Epic 28 | 分析失败韧性 — 错误持久化 + 重启恢复 + 内容审核跳过重试 + 层级优化超时保护 | 4 | done |
| Epic 29 | 地图可读性优化 — 冲突开关 + 同名异地过滤 + 提及数过滤 + 层级折叠展开 + 同名地点消歧 | 3 | done |
| Epic 30 | 地图可视化引擎升级 — 标签多锚点偏移 + 模拟退火导出 + 河流水系 + Whittaker 生物群落 + 轨迹动画增强 | 5 | done |
| Epic 31 | 手绘风格地图集成 — rough.js 领地/河流/海岸线/晕影渲染 | 1 | done |
| Epic 32 | 地点层级质量深度优化 — 上下文锚点注入 + 微小地点修剪 + 层级合并修复 + LLM 验证 + 重建超时保护 + 子图分治验证 + 兄弟聚类 + 宏观骨架预生成 + 家具噪声过滤 + 同义地名合并 + 同后缀兄弟增强 | 6 | done |
| Epic 33 | 地图绘制三项优化 — 海岸线/地形纹理稳定性 + 高密度子节点自适应分散 + 大领地 hachure 填充去除 | 3 | done |
| Epic 34 | 百科大版本升级 — 列表章节数/层级标签/热度排序 + 定义搜索 + 空间关系面板 + 人物足迹 + 组织当前成员/彩色关系 + 概念章节跳转 + 跨页导航 + 场景索引 + 冲突标记 + 世界观浏览 + 迷你位置图 | 13 | done |
| Epic 35 | 时间线大版本升级 — 智能降噪(物品/角色过滤) + 关系变化事件生成 + 场景情绪链接 + 泳道过滤 + 低重要度自动折叠 + 默认隐藏低价值类型 | 9 | done |
| Epic 36 | 关系图大版本升级 — 关系规范化扩展(70+映射) + 六大分类着色 + 分类过滤芯片 + 组织归属推断 + 统计图例 + 暗色模式适配 | 9 | done |
| Epic 37 | 阅读页大版本升级 — 高亮开关 + 阅读进度 + 章节字数 + 场景错误提示 + 实体卡片错误/缓存 + 目录计数 + 快捷键 + 实体类型过滤 + 书签系统 + 章节预加载 + 响应式卡片 + 场景搜索过滤 | 16 | done |
| Epic 38 | 层级质量深度修复 — 同层兄弟主设定守卫 + 外文名兄弟检测 + 合并安全网 | 3 | done |
| Epic 39 | 书架大版本升级 — 搜索提示 + 分析/阅读进度文字 + 字数排序 + 上次阅读时间 + novelStore 重构 + 删除确认数据量 + 缓存过期提示 + 拖拽上传 + 键盘快捷键 + 上传进度条 + 导入导出入口 + 数据库事务保护 | 13 | done |
| Epic 40 | 导出功能大版本升级 — 关系链震荡去重 + 导出v3用户数据(书签/地图/世界观) + app_version动态化 + file_hash冲突检测 + 实体字典过滤 + 物品噪声过滤 + 模板全格式可用 + 章节范围选择 + Timeline限制提升+噪声过滤 + 导出进度 + 批量对话导出 + 文件名增强 | 12 | done |

共计 **40 个 Epic、208 个 Story**，全部完成。
