...
- **多层级世界地图** — 宏观区域划分（如四大部洲）、多空间层（天界/冥界/洞府副本）、传送门连接、区域内约束求解布局、程序化地形生成、人物轨迹动画、三态战争迷雾、用户可编辑世界结构、16:9 宽画布、智能副本合并、地理上下文面板（Tab 切换）、场景转换分析增强地点层级、两步层级重建（预览变更→确认应用）、嵌套 Convex Hull 领地轮廓（手绘风格边缘扰动）、地形语义纹理层（山脉三角/河流波浪/森林树丛/沙漠点阵/洞穴钟乳石，按地点 icon 自动散布）
...
| Epic 25 | 地图视觉改进 — Q1-Q6 Quick Wins + M1 嵌套 Hull 领地 + M2 地形语义纹理 | 3 | done |
