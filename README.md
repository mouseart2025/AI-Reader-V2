...
- **多层级世界地图** — 宏观区域划分（如四大部洲）、多空间层（天界/冥界/洞府副本）、传送门连接、区域内约束求解布局、程序化地形生成（Whittaker 二维生物群落矩阵 + Lloyd 松弛 + OpenSimplex 噪声 + 河流水系网络）、人物轨迹动画（渐进式路径绘制 + 脉冲标记 + 三档变速 + 自动跟随 + 停留时长圆点 + 章节标注）、标签智能防碰撞（8 候选锚点尝试 + 模拟退火全局优化导出）、三态战争迷雾、用户可编辑世界结构、16:9 宽画布、智能副本合并、地理上下文面板（Tab 切换）、场景转换分析增强地点层级、两步层级重建（预览变更→确认应用）、嵌套 Convex Hull 领地轮廓（手绘风格边缘扰动）、地形语义纹理层（山脉三角/河流波浪/森林树丛/沙漠点阵/洞穴钟乳石，按地点 icon 自动散布）、暗色层专属大气纹理（天界星空/冥界岩雾/海底波浪/副本漩涡/灵界紫雾）、区域名称弯曲文字（SVG textPath 沿弧线排列）、地点点击直接打开实体卡片、高清 PNG 地图导出
